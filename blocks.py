...
class Spring(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = image.load("%s/blocks/spring.png" % ICON_DIR)
...
class BlockTree(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = image.load("%s/blocks/block_tree.png" % ICON_DIR)
        self.image = transform.scale(self.image, (32, 32))  # Изменяем размер на 32x32
        self.rect = Rect(x, y, 32, 32)  # Также изменяем размер прямоугольника
class InvisibleDeadlyBlock(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = Surface((32, 32))
        self.image.set_alpha(0)  # Делаем блок полностью прозрачным
...
class LavaBlock(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = image.load("%s/blocks/lava_block.png" % ICON_DIR)
        self.image = transform.scale(self.image, (32, 32))  # Используем размер 32x32
        self.rect = Rect(x, y, 32, 32)

class CloudBlock(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = image.load("%s/blocks/Cloud_block.png" % ICON_DIR)
        self.image = transform.scale(self.image, (96, 64))  # Используем размер 64x96
        self.rect = Rect(x, y, 96, 64)

class CloudSpringBlock(sprite.Sprite):
    def __init__(self, x, y):
        sprite.Sprite.__init__(self)
        self.image = image.load("%s/blocks/Cloud_block_2.png" % ICON_DIR)
        self.image = transform.scale(self.image, (96, 64))  # Используем размер 96x64
        self.rect = Rect(x, y, 96, 64)
        self.boost_power = 35  # Сила подброса (можно настроить)
...
class BlockTeleport(sprite.Sprite):
    def __init__(self, x, y, teleport_id):
        ...
        self.image = image.load("%s/blocks/portal.png" % ICON_DIR)
...
class Letter(pygame.sprite.Sprite):
    def __init__(self, x, y, letter_number):
        ...
        original_image = pygame.image.load(os.path.join(ICON_DIR, "blocks", "letter.jpg"))
    ...
    def get_number_image(self):
        image_path = os.path.join(ICON_DIR, "blocks", f"{self.letter_number}_number.jpg")
        try:
            ...
            placeholder.fill((200, 200, 200))  # Серый квадрат как заполнитель
            return placeholder
